---
name: meimei 情绪分析小程序 MVP 方案 v12（定稿版）
...
# meimei 情绪分析小程序 方案 v12（定稿版）

历经九轮问答 + 两轮心理专家评审（第一轮 7 条全采纳；第二轮 4 条，其中"异议处理"按用户修正为平等讨论后判断）定稿。本文件是施工蓝图：任何改动先改方案，再动工。
...
- 用户质疑分析结果时，AI 与用户**平等讨论**：温柔解释依据、就事论事，不诡辩、不迎合；
  讨论后由 AI 判断是否改分；用户的异议内容存入记录（用户的自我认知同样珍贵）
- 不主动收尾：用户不走，对话就一直继续
