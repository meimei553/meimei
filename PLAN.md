---
name: meimei 情绪分析小程序 MVP 方案 v33（定稿版·已敲定）
...
# meimei 情绪分析小程序 方案 v33（定稿版·已敲定）

（v33：v32 新功能引入问题的三视角复审，6 条全部采纳——声明与实现一致、
示例原创虚构、猜想环节极轻化可跳过、范围膨胀管理、猜错包装文案）
...
- 全局规则已固化为 `.cursor/rules/meimei.mdc`（每次新对话自动加载）：计划先行、编码规范、架构约束，详见该文件
...
89. 猜想环节极轻化：只选"你觉得最主要的情绪是哪一个" + 强度滑块（轻/中/重），一键完成，不做百分比分配
