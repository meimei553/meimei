---
name: meimei 情绪分析小程序 MVP 方案 v41（定稿版·已敲定）
...
# meimei 情绪分析小程序 方案 v41（定稿版·已敲定）

（v41：用户点名：上架缺的那些（服务器/HTTPS/备案、登录隔离、改隐私声明、合规显示、付费模型、UI 过审、三期个体户）到时候必须逐步教怎么做，不能只列名词。现在先联调。
...
- v41：上架缺项到节点时必须**逐步教操作**（用户点名）；当前任务是本机联调，不是上架
...
【上架时逐步教学】（v41，用户点名：记得到时候教我怎么做，提醒我。
  触发时机：二期开始部署/登录、用户说要上架、准备交微信审核。
  要求：按条逐步带操作，零基础能照着点；禁止只丢名词清单就结束。）
1. 公网服务器上跑后端 + HTTPS + 域名 ICP 备案 + 小程序后台配置 request 合法域名（开发期「不校验合法域名」正式版无效）
2. 微信登录 / 账户绑定，记录按用户隔离（禁止继续单库混用）
3. 修改隐私政策与弹窗：上架后不能再写「数据不出你的设备」；改完重新让用户同意
4. 上面「小程序合规显示待办」1～6 全部做进小程序
5. 把模型从免费 glm-4.7-flash 切到付费版，并一起核对智谱付费规则
6. UI 做到可过审（含 ECharts 雷达图，不再是文字骨架）
7. 小程序后台填写名称/简介/类目/截图/隐私指引/生成式 AI 声明，再提交审核
8. 三期做充值时才教：注册个体工商户、微信支付虚拟商品（现在上架个人主体即可，先不用开公司）
